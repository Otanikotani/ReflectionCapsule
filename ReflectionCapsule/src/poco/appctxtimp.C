...
//
int POCO_AppContextImpl::parseDoc(
	POCO_DOM::Element* doc, POCO_AppEnv* env)
{
	if( is_closed ) {
...
int POCO_AppContextImpl::resolve_all_bean_ids(
	POCO_DOM::Element* elem, POCO_AppEnv* env)
{
	...
	if( strcmp(tag, TOKEN_BEAN) == 0 ) {
		...
		if( id != NULL ) {
			...
			desc->bean_class.replace('}', '>');
			desc->bean_class_ptr
				= POCO_Util::class_to_ptr(desc->bean_class.in());
			attr = elem->getAttribute(TOKEN_ABSTRACT);
			if( attr != NULL && strcmp(attr, TOKEN_TRUE) == 0 ) {
				desc->is_abstract = 1;
			}
...
static POCO_factory_wrapper_t resolve_factory_wrapper(const char* id);

POCO_BeanDesc* POCO_AppContextImpl::parseBean(
	POCO_DOM::Element* elem, POCO_AppEnv* env)
{
	...
	if( attr != NULL ) {
		...
		if( factory == desc ) {
			POCO_AppEnvImpl::fatal_error(env, 
				"[E007] a bean is used as factory of itself",
				elem);
		...
		if( factory->is_abstract ) {
			POCO_AppEnvImpl::fatal_error(env, 
				"[E009] factory bean is abstract", elem);
			return NULL;
		}
	...
	if( attr != NULL ) {
		...
		if( desc->bean_class.in() != NULL 
		 && desc->bean_class.in()[0] == '*' ) {
			POCO_AppEnvImpl::fatal_error(env, 
				"[E010] missing factory name or "
	...
	if( attr != NULL ) {
		...
		if( desc->the_wrapper == NULL ) {
			POCO_String msg = (const char*)"[E047] fail to resolve factory wrapper of id: ";
			msg += attr;
	...
	if( lazy_resolve == 0 
	 && desc->factory_method_name.in() != NULL
...
int POCO_AppContextImpl::parseMethodArgs(
	POCO_DOM::Element* elem, POCO_BeanDesc* desc, POCO_BeanDesc* this_bean, POCO_AppEnv* env)
{
	POCO_Table table;
...
int POCO_AppContextImpl::parsePropSetters(
	POCO_DOM::Element* elem, POCO_BeanDesc* desc, POCO_AppEnv* env)
{
	POCO_Table table;
...
POCO_PropDesc* POCO_AppContextImpl::parsePropSetter(
	POCO_DOM::Element* elem, POCO_BeanDesc* desc, POCO_AppEnv* env)
{
	...
	if( desc->bean_class_ptr.in() != NULL ) {
		...
		if( strcmp(target, TOKEN_NONE) != 0 ) {
			POCO_AppEnvImpl::fatal_error(env, "[E012] invalide target attribute value", elem);
			return NULL;
		}
		else {
			prop->is_no_target = 1;
...
POCO_PropDesc* POCO_AppContextImpl::parseOnError(
	POCO_DOM::Element* elem, POCO_BeanDesc* desc, POCO_AppEnv* env)
{
	const char* attr = elem->getAttribute(TOKEN_IOC_METHOD);
	if( attr == NULL ) {
...
int POCO_AppContextImpl::parseDupMethod(
	POCO_DOM::Element* elem, POCO_BeanDesc* desc, POCO_AppEnv* env)
{
	desc->is_self_dup = 1;
	...
	if( desc->bean_class_ptr.in() == NULL ) {
		POCO_AppEnvImpl::fatal_error(env, 
			"[E015] dup-method is specified on "
	...
	if( desc->is_self_dup ) {
		...
		desc->dup_method = POCO_Reflection::get_method(
			NULL, desc->bean_class_ptr.in(), signature.in());
	}
	else {
		signature = desc->dup_method_name.in();
		signature += "(";
...
int POCO_AppContextImpl::parseReleaseMethod(
	POCO_DOM::Element* elem, POCO_BeanDesc* desc, POCO_AppEnv* env)
{
	int self_destroy = 1;
	...
	if( self_destroy ) {
		...
		if( is_void_bean ) {
			POCO_AppEnvImpl::fatal_error(env,
                        "[E017] self destroy method is specified on a void bean", elem);
                	return 0;
		}
	}
	else {
	...
	if( attr == NULL ) {
		...
		signature += ")";
		desc->destroy_method = POCO_Reflection::get_method(
			NULL, NULL, signature.in());

		if( desc->destroy_method == NULL ) {
			POCO_String msg = (const char*)
				"[E018] fail to resolve the dynamic "
		...
		if( dtor_bean == desc ) {
			...
			signature += ")";
			desc->destroy_method = POCO_Reflection::get_method(
				NULL, dtor_bean->bean_class_ptr.in(), signature.in());
...
POCO_ArgDesc* POCO_AppContextImpl::parseArg(
		POCO_DOM_Element* elem, POCO_BeanDesc* this_bean, POCO_AppEnv* env)
{
	POCO_ArgDesc* desc = new POCO_ArgDesc();

	desc->is_index_arg =
		(strcmp(elem->getTagName(), TOKEN_INDEX_ARG) == 0);
	...
	if( desc->is_bean ) {
		...
		if( strcmp(attr, TOKEN_DUP) == 0 ) {
			desc->pass_syntax = POCO_ArgDesc::PASS_DUP;
		}
		else
		if( strcmp(attr, TOKEN_DEREF) == 0 ) {
			desc->pass_syntax = POCO_ArgDesc::PASS_DEREF;
		}

		desc->valdesc = parseBeanRef(elem, desc->class_ptr.in(), this_bean, env);
	...
	if( desc->is_array ) {
		...
		if( child == NULL
		 || strcmp(child->getTagName(), TOKEN_ARRAY) != 0 ) {
			delete desc;
			POCO_AppEnvImpl::fatal_error(env, 
		...
		if( strcmp(desc->type.in(), TOKEN_NAMEVALUE) == 0 ) {
		...
		if( !is_namevalue_array && !is_bean_array && attr && env->getArray(attr) ) {
			...
			for(int i=0; i<child->numOfChildElements(); i++) {
				...
				if( is_bean_array ) {
					vdesc = parseBeanRef(item, desc->class_ptr.in(), this_bean, env);
				}
				else {
...
POCO_ValDesc* POCO_AppContextImpl::parseBeanRef(
	POCO_DOM::Element* elem, const char* class_ptr, POCO_BeanDesc* this_bean, POCO_AppEnv* env)
{
	...
	if( attr != NULL ) {
		...
		if( bean == NULL ) {
			POCO_AppEnvImpl::fatal_error(env, "[E022] the bean ref'ed by the 'ref' "
		...
		if( child == NULL ) {
			...
			if( strcmp(tag, TOKEN_REF) == 0 ) {
				...
				if( bean == NULL ) {
					POCO_AppEnvImpl::fatal_error(env, "[E024] the bean ref'ed by "
			...
			if( strcmp(tag, TOKEN_THIS_BEAN) == 0 ) {
				if( this_bean == NULL ) {
					POCO_AppEnvImpl::fatal_error(env, 
				...
				POCO_AppEnvImpl::fatal_error(env, "[E025] invalid child element", elem);
				return NULL;
			}
		}
	}
	...
	if( bean != NULL ) {
		if( class_ptr != NULL && strcmp(class_ptr, bean->bean_class_ptr.in()) != 0 ) {

			val->cast_method = POCO_Reflection::get_typecast_method(
					class_ptr, bean->bean_class_ptr.in());

			if( val->cast_method == NULL ) {
				POCO_String msg = (const char*)
					"[E026] fail to resolve the "
...
POCO_ValDesc* POCO_AppContextImpl::parseValue(
...
POCO_ValDesc* POCO_AppContextImpl::getAndConvertValue(
	const char* attr, POCO_DOM::Element* elem,
	const char* type, POCO_AppEnv* env)
{
	POCO_ValDesc* val = new POCO_ValDesc;

	if( strcmp(type, TOKEN_BYTE) == 0
	 || strcmp(type, TOKEN_UCHAR) == 0 ) {
	...
	if( elem && strcmp(type, TOKEN_NAMEVALUE) == 0 ) {
		...
		delete val;
		POCO_AppEnvImpl::fatal_error(env, 
			"[E028] the value of 'type' attribute is invalid",
			elem);
		return NULL;
	}
	...
	delete val;
	POCO_AppEnvImpl::fatal_error(env, 
			"[E029] the value of the 'value' attribute is invalid",
			elem);
	return NULL;
}

POCO_BeanDesc* POCO_AppContextImpl::getBeanDesc(const char* id, POCO_AppEnv* env)
{
	if( is_closed ) {
		POCO_AppEnvImpl::fatal_error(env, "[E030] application context closed already");
		return NULL;
	}
	...
	for(int i=0; i<imported_ctxts.count();i++) {
		POCO_AppContextImpl* ctxt
			= (POCO_AppContextImpl*)imported_ctxts.get(i);
	...
	if( desc == NULL ) {
		POCO_AppEnvImpl::fatal_error(env, "[E032] bean with the specified id not found");
	}

	return desc;
}

void* POCO_AppContextImpl::getBean(const char* id, POCO_AppEnv* env)
{
	if( env == NULL ) {
...
void* POCO_AppContextImpl::getBean(const char* id,
				const char* class_name, POCO_AppEnv* env)
{
	if( env == NULL ) {
		env = POCO_AppContext::getDefaultAppEnv();
	}
	...
	if( class_ptr_name.in() == NULL ) {
		POCO_AppEnvImpl::fatal_error(env, "[E033] invalide bean id in calling getBean()");
		return NULL;
	}
	...
	if( strcmp(desc->bean_class_ptr.in(), class_ptr_name.in()) != 0 ) {
		POCO_proxy_t typecast_method
			= POCO_Reflection::get_typecast_method(
						class_ptr_name.in(),
						desc->bean_class_ptr.in());
...
const char* POCO_AppContextImpl::getBeanPtrTypeId(const char* id, POCO_AppEnv* env)
{
	if( env == NULL ) {
		env = POCO_AppContext::getDefaultAppEnv();
	}
...
int POCO_AppContextImpl::initSingletons(POCO_AppEnv* env)
{
	if( env == NULL ) {
		env = POCO_AppContext::getDefaultAppEnv();
	}
...
int POCO_AppContextImpl::terminate(POCO_AppEnv* env)
{
	if( env == NULL ) {
		env = POCO_AppContext::getDefaultAppEnv();
	}
...
int POCO_AppContextImpl::parseImport(
		POCO_DOM::Element* elem, POCO_AppEnv* env)
{
	...
	if( imported_dom != NULL ) {
		...
		if( res_type == NULL || strcmp(res_type, "file") == 0 ) {
			...
			// todo -- Ke
			POCO_String msg = (const char*)"[E300] unsupported import resource type \"";
			msg += res_type;
			msg += "\"";
			POCO_AppEnvImpl::fatal_error(env, msg.in(), elem);
		}
...
int POCO_AppContextImpl::parseLoadLib(
		POCO_DOM::Element* elem, POCO_AppEnv* env)
{
...
static POCO_Hashtable& _desc_readers()
{
...
extern "C" {
int pocomatic_register_desc_reader(POCO_AppDescReader* reader)
{
	if( reader == NULL ) {
		return 0;
	}
...
{
	POCO_AppDescReader* reader
		= (POCO_AppDescReader*)_desc_readers().get(tid);
...
POCO_AppContextImpl::create_from_file(const char* loc, POCO_AppEnv* env)
{
	...
	if( strm == NULL ) {
		POCO_String msg = (const char*)"[E043] fail to open the descriptor file ";
		msg += loc;
	...
	if( strncmp(buf, "pococtx:", 8) == 0 ) {
		tid = "ctx";
	}
	else {
		POCO_String msg = (const char*)"[E041] the type of the descriptor in file ";
		msg += loc;
...
POCO_AppContextImpl*
POCO_AppContextImpl::create_from_string(
...
void POCO_AppContextImpl::add_bean(POCO_BeanDesc* bean)
{
	all_beans.put(bean);
}

void POCO_AppContextImpl::push_inited_singleton(POCO_BeanDesc* bean)
{
	bean_term_stack.put(bean);
}

void POCO_AppContextImpl::add_keyed_bean(POCO_BeanDesc* bean)
{
	keyed_beans.put(bean->the_key.in(), bean);
}

POCO_BeanDesc* POCO_AppContextImpl::get_keyed_bean(const char* key)
{
	return (POCO_BeanDesc*)keyed_beans.get(key);
}

void POCO_AppContextImpl::destroy()
{
...
POCO_factory_wrapper_t resolve_factory_wrapper(const char* id)
{
	return (POCO_factory_wrapper_t)factory_wrappers.get(id);
}

int pocomatic_register_factory_wrapper(const char* id, POCO_factory_wrapper_t wrapper)
{
